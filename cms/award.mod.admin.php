    ...
    function admAward() 
    {
        ...
        if($ext=="ajax")
        {
            ...
            {    
                ...
                $mess=Award::loadImage($_FILES["file"]);
                if($mess=="OK")
                {
                    $imgdir = "/site/design/img/award/";
                    $GLOBALS['_RESULT'] = array(
                                                "image" => $imgname,
                                                "image_small" => $imgnamesmall,    
                                                "mess" => $mess,
                                                ); 
                }
            }
...
//            {
	            if($com=="view")
                {
                    $row=Award::artListData($category, $order, $strim);
                    $tpl->assign('listing',$row);
                    $c_cont=$tpl->get("award-list");	
                }
                if($com=="add")
                {
                    $c_cont=$tpl->get("award-add-edit");	
                }
                if($com=="edit")
                {
	                $cat_data=Award::catData($id);
                    $tpl->assign('cat_data',$cat_data);
                    $c_cont=$tpl->get("award-add-edit");	
                }
                if($com=="update")
                {   
//$query="SELECT * FROM `mr_award_category` WHERE `name`='{$_GET['type']}' LIMIT 1";
//$res = mysql_query($query);
//while($row = mysql_fetch_array($res)) 
//{
//   $seolink = trim($row['seolink']);   
//}
$query="SELECT * FROM `mr_award` WHERE `id`='{$id}' LIMIT 1";
$res = mysql_query($query);
while($row = mysql_fetch_array($res)) 
{
    $old_img = $row['image'];   
}
if($old_img == $image || $old_img == "")
{
    $arr_value['image'] = $image;
}
else
{
    $arr_value['image']=$old_img.",".$image;
}
if($img !== NULL)
{
    $immgg = explode(",",$arr_value['image']);
    if($key = array_search($img,$immgg))
    {
       unset($immgg[$key]);
    }
   $arr_value['image'] = implode(",",$immgg);
}
//echo "old_img - ".$old_img."<br />".$arr_value['image'];
                                                 
                    $arr_value['type']=$type;
                    //$arr_value['category_id']=$category;
                   ...
                   // $arr_value['time']=isset($date) && $date!==""?$date:date("Y-m-d H:i:s");
                    $arr_value['caption']=Text::cut($caption);
...
unset($_SESSION);
    ...
    function Navigate()
    {
        ...
        $art_navigate=$tpl->get('award-navigate');
    ...
    function loadImage($arr)
    {
        foreach ($_REQUEST as $key=>$val)
        {
            ...
            $uploaddir = SITE_PATH."/site/design/img/award/";
